...
int main() {
    ...
    for(i = 0; i < n_czy; i++) {
        id_czy[i] = i+1;
        pthread_create(&czytelnicy[i], NULL, funkcja_czytelnika, (void *)&id_czy[i]);
    }
    for(i = 0; i < n_pis; i++) pthread_join( pisarze[i], NULL);
    for(i = 0; i < n_czy; i++) pthread_join( czytelnicy[i], NULL);
}
