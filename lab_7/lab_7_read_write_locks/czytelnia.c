...
int my_read_lock_lock(czytelnia_t* czytelnia_p, int id) {
       pthread_mutex_lock(&czytelnia_p->liczba_pisz_mut); // Nie chce żeby mi ktoś go nadpisał wiec write lock
       pthread_mutex_lock(&czytelnia_p->kolejka_mut_czyt); // Nie chce ani read , ani write
       ...
       if(czytelnia_p->liczba_pisz > 0 || czytelnia_p->kolejka_czyt > 0) {
           ...
           printf("Czytelnik %d ustawił się w kolejce do czytania! Jest %d w kolejce.\n",id, czytelnia_p->kolejka_czyt+1);
...
int my_write_lock_lock(czytelnia_t* czytelnia_p, int id) {
     ...
     pthread_mutex_lock(&czytelnia_p->liczba_czyt_mut);
     int odblokowalem_w_petli = 0;
     if(czytelnia_p->liczba_pisz > 0 || czytelnia_p->kolejka_pis > 0 || czytelnia_p->liczba_czyt > 0) {
         pthread_mutex_unlock(&czytelnia_p->liczba_pisz_mut);
         pthread_mutex_unlock(&czytelnia_p->liczba_czyt_mut);
         odblokowalem_w_petli = 1;
     ...
     if(odblokowalem_w_petli == 1) pthread_mutex_lock(&czytelnia_p->liczba_pisz_mut);
